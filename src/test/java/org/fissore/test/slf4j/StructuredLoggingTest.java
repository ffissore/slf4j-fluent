...
public class StructuredLoggingTest {
  ...
  public void structuredLogging() {
    FluentLogger logger = FluentLoggerFactory.getLogger("org.fissore.slf4j.json.error.Test");

    MDC.put("mdc1", "mdc value 1");
    MDC.put("mdc2", "mdc value 2");

    logger.error().log("log message {}", value("name", "value"));
    logger.error().log("log message {}", keyValue("name", "value"));
    logger.error().log("log message", keyValue("name", "value"));
    logger.error().log("log message {} {}", keyValue("name1", "value1"), keyValue("name2", "value2"));
    logger.error().log("log message {}", keyValue("name", "value", "{0}=[{1}]"));
    Map<String, String> myMap = new HashMap<>();
    myMap.put("name1", "value1");
    myMap.put("name2", "value2");
    logger.error().log("log message {}", entries(myMap));
    logger.error().log("log message {}", array("array", 1, 2, 3));
    logger.error(append("name", "value")).log("log message");
